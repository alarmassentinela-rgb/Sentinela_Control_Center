...
- **`cfdi_status`** Selection: `draft` (Borrador) · `pending` (Pendiente de Timbrado) · `valid` (Timbrado Válido) · `cancel` (Cancelado) · `error` (Error). Default `draft`, `tracking=True`.
  - ⚠️ El valor `cancel` está declarado pero **NO hay flujo que lo asigne** — no existe cancelación ante el PAC en el código (ver Trampas).
...
- **`cfdi_status='cancel'` es decorativo:** no existe flujo de cancelación ante el PAC; el estado nunca se setea automáticamente.
...
- **Wizards:** ninguno propio (reutiliza `account.move.reversal` estándar).
