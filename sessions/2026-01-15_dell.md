...
- **Documentation:** Created [SSH Connection Manual](../docs/manuales/conexion_ssh_aleasystems.md) and mapped the project structure in `index.md`.
...
- **Comodato "Locks" (Candados):**
  ...
  - **Penalty Field:** Added `penalty_amount` ("Penalización por Cancelación Anticipada") visible only for forced contracts.
...
- **Pre-Invoicing Flow (5 Days Before):**
  ...
  - **Action:** Generates a **Sale Order (Cotización)** 5 days before the `next_billing_date` and sends it via email.
