...
def parse_contact_id(data):
    # Regex flexible para permitir cuenta y zona de 1 a 10 caracteres
    pattern = r"\[(\w{1,10})\s+(\w{2})\s+([E|R])(\w{3})\s+(\w{2})\s+(\w{1,10})\]"
    ...
    try:
        ...
        while True:
            try:
                ...
                raw_clean = data.strip()
                logging.info(f"[RAW] {raw_clean}")
                
                cid = parse_contact_id(raw_clean)
                if cid:
                    conn.send(b'\x06') # ACK inmediato para evitar timeout del cliente
                    logging.info(f"[ACK] Enviado para {cid['account']}")
                    
                    try:
                        process_signal(cid, raw_clean)
                    except Exception as e:
                        logging.error(f"Error procesando señal en Odoo: {e}")
                else:
                    logging.warning(f"[ERROR] Trama no reconocida: {raw_clean}")
                    # Enviar ACK de todas formas para debugging si se desea, o cerrar
                    # conn.send(b'\x06') 
