...
- **⚠️ ClaveProdServ SAT (`l10n_mx_edi_code_sat`, campo de `sentinela_cfdi_prodigia` que usa el TIMBRADO):** el `_syscom_extract_enrichment` guarda `sat_description`/`clave_unidad_sat` en campos `syscom_*` **informativos**, NO en el campo del CFDI. El **wizard** de import sí mapea `sat_key → l10n_mx_edi_code_sat` (`syscom_import_wizard.py:200`). **v18.0.1.8.2 (26-jun):** el **cron** también lo llena ahora — en la Fase 1 (barrido, desde `p['sat_key']` del listado) y Fase 2 (refresh, desde `dj['sat_key']` del detalle), **solo si el campo está vacío** (no pisa claves manuales) y con guarda `'l10n_mx_edi_code_sat' in rec._fields`. Tanto el listado como el detalle traen `sat_key` (ej. batería LK5.512 → `26111701`). Sin esto, productos existentes sin clave salían con `01010101` → el SAT rechaza el 8% frontera (`[CFDI40999]`).
