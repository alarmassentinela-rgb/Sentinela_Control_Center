...
- **Versión actual:** ver `__manifest__.py` (`version`). Hoy `18.0.1.3.0`.
...
|---|---|---|
| `_inherit product.template` (`ProductTemplate`) | `models/product.py` | Campos `syscom_*` (id, modelo, marca, stock, precios USD, descontinuado) + cron `_cron_update_syscom_products` |
...
## Flujos importantes
- **Importación de catálogo (wizard):** `syscom.import.wizard` → `action_search_and_import()` consulta `/productos?busqueda=`, mapea JSON Syscom → `product.template` (`_import_single_product`), crea/actualiza jerarquía de categorías (`_get_or_create_category`, ordenada por `nivel` para crear padres primero, vincula por `syscom_category_id`), descarga imagen (`img_portada`→`image_1920`), setea `type='consu'` y `l10n_mx_edi_code_sat`.
