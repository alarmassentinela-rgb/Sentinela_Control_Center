...
class Post(Base):
    ...
    author_id: Mapped[uuid.UUID] = mapped_column(pgUUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=True)
    ...
    post_type: Mapped[str] = mapped_column(String(30), default="regular", nullable=True)
    visibility: Mapped[str] = mapped_column(String(20), default="group", nullable=True)
    comments_count: Mapped[int] = mapped_column(Integer, default=0, nullable=True)
    reactions_count: Mapped[int] = mapped_column(Integer, default=0, nullable=True)
...
class PostMedia(Base):
    ...
    post_id: Mapped[uuid.UUID] = mapped_column(pgUUID(as_uuid=True), ForeignKey("posts.id", ondelete="CASCADE"), nullable=True)
    media_type: Mapped[Optional[str]] = mapped_column(String(10))
    url: Mapped[str] = mapped_column(String(500), nullable=False)
    thumbnail_url: Mapped[Optional[str]] = mapped_column(String(500))
    order_index: Mapped[int] = mapped_column(Integer, default=0, nullable=True)
    created_at: Mapped[Optional[datetime]] = mapped_column(TIMESTAMP(timezone=True), server_default=func.now())
