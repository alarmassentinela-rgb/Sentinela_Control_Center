...
from sqlalchemy import String, Boolean, Integer, Numeric, Text, ForeignKey, UniqueConstraint, Date, Time, Index, text
...
class TeeTimeSlot(Base):
    ...
    __table_args__ = (
        UniqueConstraint("club_id", "date", "time", name="tee_time_slots_club_date_time_key"),
        Index("idx_tee_time_slots_club_date", "club_id", "date"),
    )
    ...
    max_players: Mapped[int] = mapped_column(Integer, default=4, nullable=True)
    available_spots: Mapped[int] = mapped_column(Integer, default=4, nullable=True)
...
class TeeTimeBooking(Base):
    ...
    slot_id: Mapped[int] = mapped_column(Integer, ForeignKey("tee_time_slots.id"), nullable=True)
    user_id: Mapped[uuid.UUID] = mapped_column(pgUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    players_count: Mapped[int] = mapped_column(Integer, default=1, nullable=True)
...
class TeeTimeBookingPlayer(Base):
    ...
    __table_args__ = (
        Index("idx_booking_players_booking", "booking_id"),
        Index("idx_booking_players_user", "user_id", postgresql_where=text("user_id IS NOT NULL")),
        Index("idx_booking_players_sponsor", "sponsor_id", postgresql_where=text("sponsor_id IS NOT NULL")),
    )
