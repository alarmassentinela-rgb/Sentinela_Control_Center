...
## Flujos importantes
- **Ingesta (Fórmula 1):** `process_signal_from_receptor(vals)` hace todo en una llamada — busca el device por `account_number`; si no existe crea una **señal en cuarentena** (no crea device ni event); si existe actualiza `last_communication`, auto-resuelve un trouble `[AUTO_OFFLINE]` abierto, resuelve la prioridad **por cuenta** (override en `sentinela.device.alarm.config` para ese device+code → prioridad global del código → default `35`; desde v18.0.1.17.1, modelo Securithor Opción A), crea evento solo si el código `requires_attention`, crea la señal siempre, **notifica al CLIENTE** si el código está marcado para avisar (`_dispatch_client_notification`, Fase 3 v18.0.1.19.0: flags efectivos = `device.alarm.config` de la cuenta → defaults del código global; canales email/Telegram/WhatsApp, fail-safe por canal; **SMS a celular pendiente de gateway**), notifica al `bus.bus` (canal `sentinela_monitoring`) y refresca `receiver.status.last_heartbeat`.
...
- **Semántica de prioridades (`sentinela.alarm.priority`, modelo Securithor Opción A desde v18.0.1.18.0):** `level` **1 = la más importante**, el número sube = menos importante. `level` es **único** (`unique(level)`) → cada prioridad es un rango total. El **sonido vive en la prioridad** (`priority_sound`), y hay una **prioridad-perfil por TIPO de alarma**. Esquema en prod (`code` → nombre): **CRIT_FIRE** Crítica·Incendio (11x) · **CRIT_INTRUSION** Crítica·Intrusión (13x) · **CRIT_MEDICAL** Crítica·Médica (100-102) · **CRIT_GEN** Crítica·General (12x/14x/15x-16x/2xx) — los 4 SLA 1 min · **FALLA** Falla/Avería (trouble+offline, 15 min, tono suave) · **RUTINA** Rutina/Info (open_close/test/status, 60 min, sin audio; `requires_attention=False` → NO crean evento). Solo `alarm`+`trouble` generan eventos. La prioridad efectiva se resuelve **por cuenta** (`device.alarm.config` → código global → default id 35); el sonido sale de esa prioridad. **NO dependas del número de `level` en código** (se renumera al agregar tipos): usa `code` estable (`code='FALLA'` en el cron offline) o el SLA (`sla<=5` = crítico en `create_fsm_order`). `alarm_service.js` suena la prioridad de menor `level` primero. El modelo tiene `active` (archivar).
