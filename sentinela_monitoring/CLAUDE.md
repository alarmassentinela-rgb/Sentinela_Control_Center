...
- **Semántica de prioridades (`sentinela.alarm.priority`, desde v18.0.1.17.0):** `level` **1 = la más importante** (sirena), el número sube = menos importante — al revés de lo que sugería el campo antes. Esquema en prod: **1 Crítica** (códigos `alarm`, sirena, SLA 1 min) · **2 Falla/Avería** (códigos `trouble` + paneles offline, tono suave, 15 min) · **3 Rutina/Info** (open_close/test/status, sin audio, 60 min; esos códigos tienen `requires_attention=False` → NO crean evento). Solo `alarm` y `trouble` generan eventos atendibles. El modelo tiene campo `active` (archivar) y `_order='level asc'`. **Si tocas lógica que compara `level`, recuerda que menor = más crítico:** el cron offline apunta a level 2, `create_fsm_order` marca urgente con `level<=1`, y `alarm_service.js` suena la prioridad de menor `priority_level` primero.
